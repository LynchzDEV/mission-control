...
export type ProviderUsage = { provider: string; fiveHour: QuotaWindow; weekly: QuotaWindow; reason: string; observedAt: string | null }
export const FRESH_MS = 3 * 60_000
export function ageLabel(observedAt: string | null, now: number = Date.now()): string | null {
  const at = observedAt === null ? NaN : Date.parse(observedAt)
  if (!Number.isFinite(at) || now - at < FRESH_MS) return null
...
export function renderUsage(host: HTMLElement, values: ProviderUsage[]): void {
  ...
  for (const data of values) {
    ...
    const age = ageLabel(data.observedAt)
    section.dataset.stale = String(age !== null)
    const period = document.createElement('span'); period.className = 'quota-period'; period.textContent = `${single ? 'Weekly' : data.fiveHour.estimate ? '5h est.' : '5h'}${age ? ` · ${age}` : ''}`
