...
async function main() {
  ...
  await runServerCheck('elysia-hello', 'docs/spikes/runtime/server-hello.ts', 7997, '/', 'hello from elysia spike')

  console.log('\n== 3a. node-pty under Bun (expected to fail, see docs/decisions/runtime-spike.md) ==')
  const ptyResult = await runScriptCheck('node-pty', 'docs/spikes/runtime/check-node-pty.ts')
  console.log(ptyResult.exitCode === 0 ? '[UNEXPECTED PASS] node-pty' : '[EXPECTED FAIL] node-pty — see decision doc for exact error')

  console.log('\n== 3b. bun-pty fallback ==')
  const bunPtyResult = await runScriptCheck('bun-pty', 'docs/spikes/runtime/check-bun-pty.ts')
  ...
  const buildResult = await runScriptCheck('bun-build', 'docs/spikes/runtime/check-bun-build.ts')
  ...
  await runServerCheck('elysia-jsx', 'docs/spikes/runtime/server-jsx.tsx', 7998, '/view', 'hello mission control')
