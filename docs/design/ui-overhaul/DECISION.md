...
The visual source is `docs/design/ui-overhaul/terminal-components.html`. Earlier component snapshots are comparison history. Earlier mineral/light application briefs and unfinished application changes are superseded visually. Terminal output and activity samples are replaced by live data; controls must use the existing backend actions.
