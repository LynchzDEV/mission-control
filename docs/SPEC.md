...
const ENGINES = {
  ...
  glm:    { cmd: 'claude', env: {
              ...
              ANTHROPIC_DEFAULT_OPUS_MODEL: 'glm-5.3[1m]',   // [1m] = Claude Code 1M-context flag, stripped before API
...
- `~/.codex/AGENTS.md` is a generated real file translated from the handwritten portion of `~/.claude/CLAUDE.md`. The continuous-improvement tail, graphify section, RTK include and dispatch-contract bullets are removed. The header states Codex permissions, worker behavior and skill locations; tool references are rewritten for execution in the current thread, and Claude hook configuration sentences are removed.
- Visible skill directories, including linked source directories, are inspected through `SKILL.md`; `.system` is never touched. Clean skills remain directory symlinks. Skills containing Claude machinery become real translated `SKILL.md` files with unchanged front matter and a provenance footer; their other entries are symlinked to preserve scripts and references. Excluded orchestration/maintenance skills and skills with more than 30% residual Claude-only body lines are skipped; stale directory symlinks for skipped skills are removed.
- Top-level Markdown agents with valid YAML front matter become `~/.codex/agents/<name>.toml`, containing name, a `# generated by mission-control` marker on the following line, description and translated body instructions. Bodies exceeding the same 30% residual threshold are skipped; an existing TOML is removed only when its first line exactly matches the source agent's generated name line and its second line is the marker. Unmarked TOML files survive this cleanup. README, AGENTS, CLAUDE, CONTRIBUTING, GEMINI and ARCHITECTURE documents are ignored; removed sources and Codex-only agents are left alone.
- Previous instruction symlinks are unlinked before writing. Handwritten instruction files and displaced personal skill directories move under `~/.codex/backup/` with `.pre-mission-control-<timestamp>` suffixes; backup directories use mode `0700`. Generated files are rewritten only when content differs, preserving unchanged mtimes.

The result reports linked, written, translated, moved-aside and skipped paths; translated paths identify generated files changed in this run. Failures and Claude-only skips have human-readable reasons and do not prevent other assets or startup from proceeding. This sync targets the personal engine configurations; worker profiles stay slim on purpose and do not receive these assets.
