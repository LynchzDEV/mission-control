...
function request(path: string, method: string, body?: unknown, origin = 'http://localhost'): Request {
...
describe('/api/flow/:label/run', () => {
  test('refuses a rebinding host', async () => {
    const { app } = build()
    expect((await app.handle(request('/api/flow/x/run', 'POST', {}, 'http://rebind.example'))).status).toBe(403)
    expect((await app.handle(request('/api/flow/x/run', 'GET', undefined, 'http://rebind.example'))).status).toBe(403)
  })

  test('400 on a malformed body, 422 with misses on a thin task', async () => {
    const { app } = build()
    const malformed = await app.handle(request('/api/flow/t/run', 'POST', { engine: 'glm', cwd: repo, preamble: PREAMBLE, tasks: [] }))
    expect(malformed.status).toBe(400)
    expect(await malformed.json()).toEqual({ error: 'tasks must be a non-empty array' })
    const thin = await app.handle(request('/api/flow/t/run', 'POST', { engine: 'glm', cwd: repo, preamble: 'x', tasks: [{ title: 'a', prompt: 'b' }] }))
  ...
  test('starts a run, reads it back, refuses a duplicate, and stops it', async () => {
    ...
    const started = await app.handle(request('/api/flow/my%20ticket/run', 'POST', payload))
    ...
    const read = await app.handle(request('/api/flow/my%20ticket/run', 'GET'))
    expect(((await read.json()) as { status: string }).status).toBe('running')
    expect((await app.handle(request('/api/flow/nope/run', 'GET'))).status).toBe(404)
    expect((await app.handle(request('/api/flow/my%20ticket/run', 'POST', payload))).status).toBe(409)

    const stopped = await app.handle(request('/api/flow/my%20ticket/run/stop', 'POST'))
    expect(stopped.status).toBe(200)
    expect(runner.get('my ticket')?.status).toBe('stopped')
    expect((await app.handle(request('/api/flow/my%20ticket/run/stop', 'POST'))).status).toBe(409)
  })
})
