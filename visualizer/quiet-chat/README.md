...
`npm exec --yes --package=vite@8.3.0 -- vite visualizer/quiet-chat --host 127.0.0.1 --port 51947 --strictPort`
...
`VITE_WORKSPACE_DIR="$PWD" npm exec --yes --package=vite@8.3.0 -- vite build visualizer/quiet-chat --base=/quiet-chat/ --outDir ../../public/quiet-chat`
...
`vite.config.js` uses Vite's [native middleware and proxy capabilities](https://vite.dev/guide/api-plugin.html#configureserver) to issue an existing Mission Control signed session as an HttpOnly, SameSite=Strict cookie on local document requests. Cookie signing stays on the server. The listener binds only 127.0.0.1:51947 and checks the actual socket peer, exact Host, Origin, and Fetch Metadata; unrelated sites, embedded pages, rebinding hosts, and foreign WebSocket origins are rejected. Terminal API/socket paths and exact jobs, flow, models, roles, and workflow-list endpoints are proxied. Those list/metadata endpoints accept GET only; write requests return 405. No password is recovered, reset, or sent to the browser. The existing app's remote authentication remains intact. This avoids restarting the app and losing its in-memory terminal registry.
...
`node visualizer/quiet-chat/check.mjs`
...
`node visualizer/quiet-chat/check-live.mjs`
...
`bun test visualizer/quiet-chat/local-access.test.ts`
...
`bun test test/awareness.test.ts visualizer/quiet-chat/local-access.test.ts`
