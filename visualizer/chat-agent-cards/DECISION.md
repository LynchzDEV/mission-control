...
**Real files to touch:** `visualizer/quiet-chat/index.html`, `style.css`, `preview.js` (design); later `client/transcript-view.ts`, `client/markdown.ts`, `client/thread-drawer.ts`, `server/threads.ts`, `/api/jobs` (production).
